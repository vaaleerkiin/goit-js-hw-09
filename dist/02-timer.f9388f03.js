...
class dateTimer {
    ...
    secondsTimer() {
        let leftTime;
        (0, _notiflixNotifyAio.Notify).success(`Timer started`);
        return timerId = setInterval(()=>{
            if (selectedUnixDate < new Date().getTime() + 1000) {
                clearInterval(timerId);
                (0, _notiflixNotifyAio.Notify).success(`Time is up`);
                isActive = false;
    ...
    writeDateInHTML(time) {
        for(const el in time)document.querySelector(`[data-${el}]`).innerHTML = `${time[el]}`.padStart(2, 0);
    }
}
